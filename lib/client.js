...
function Client(options) {
  ...
  this.MLT_PATH = 'mlt';
  this.UPDATE_PATH = 'update/json/docs';
  this.UPDATE_BATCH_PATH = 'update';
 ...
 */
Client.prototype._callSolrServer = async function (requestFullPath, fetchOptions, finalCallback) {
  const result = await fetch(requestFullPath, fetchOptions)
  ...
  try {
    ...
    if (typeof finalCallback === 'function') {
      return finalCallback(null,json)
    } else {
      return error
    }
  } catch (err) {
    if (typeof finalCallback === 'function') {
      return finalCallback(err.message)
    }
  }
};
 ...
 * @returns {Undefined|Promise}
 */
Client.prototype.updateBatch = function(data, options, finalCallback) {
  var bodyData;
  if (arguments.length < 3 && _.isFunction(options)) {
    finalCallback = options;
    options = { commit: true }; // 'commit:true' option is default
  ...
  return this._requestPost(this.UPDATE_BATCH_PATH, bodyData, options, finalCallback);
};
